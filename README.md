...
│   ├── graph.py                  # Graph generators (ER, BA, WS, custom) — dense, legacy
│   ├── propagation.py            # Tier 1: deterministic synchronous propagation — legacy
│   ├── stochastic_propagation.py # Tier 2: logistic-probability stochastic propagation — legacy
│   ├── monte_carlo.py            # Monte Carlo harness (SeedSequence-based) — legacy
...
│   │
│   ├── ingestion.py              # ★ NEW: CSV → integer IDs via pd.factorize()
│   ├── graph_sparse.py           # ★ NEW: CSR sparse graph build + pickle lazy cache
│   ├── propagation_fast.py       # ★ NEW: sparse matvec + Numba JIT @njit(parallel=True)
│   ├── monte_carlo_parallel.py   # ★ NEW: 4-core ProcessPoolExecutor Monte Carlo
│   ├── progress.py               # ★ NEW: thread-safe ProgressTracker + callback protocol
│   │
...
# Install core engine dependencies (includes numba, joblib for the fast path)
...
>
> **Numba note:** On first import, `propagation_fast.py` spawns a background thread that
> pre-compiles the JIT kernel. This takes ~1–2 seconds once and is then cached by Numba's
> disk cache (`__pycache__`). Subsequent imports are instant.

---

## 2a. High-Performance Path (100K+ nodes / 1M+ edges)

The five `★ NEW` files in `cascade_engine/` form a parallel fast path built for real-world
CSV data at scale. The legacy API (`propagation.py`, `monte_carlo.py`, etc.) is unchanged.
...
# 1. Ingest CSV — maps all string labels to compact int64 IDs once
ir = ingest_edgelist(
    "storm_events.csv",
    composite_cols=["STATE", "WFO", "EPISODE_ID"],   # composite node key
    indicator_cols=["DEATHS_DIRECT"],                 # optional per-node scores
)

# 2. Build CSR sparse graph (or load from cache if edges haven't changed)
A_T, in_degree = build_or_load_sparse_graph(
    ir.source_ids, ir.target_ids, ir.n_nodes,
    edge_hash=ir.edge_hash,           # cache key — invalidated only by edge changes
    cache_dir=".cascade_cache",
)

# 3. Run deterministic cascade (Numba JIT, all 4 cores)
n = ir.n_nodes
...
final, steps, history, converged = run_until_stable_fast(
    S0, A_T, in_degree, theta_deg, theta_fail,
    progress_callback=print_callback,
)

# 4. Run stochastic Monte Carlo across 4 CPU cores
result = run_monte_carlo_parallel(
    A_T, in_degree, theta_deg, theta_fail,
    seed_node=0, trials=500, seed=42,
    n_workers=4,
    progress_callback=print_callback,
)
print(result.summary_dict())

# 5. Resolve integer IDs back to original labels (UI reporting only)
from cascade_engine.ingestion import load_lookup_table
lookup = ir.lookup_table   # or load_lookup_table("lookup.json")
print(lookup[str(0)])      # → e.g. "OH|ILN|42"
```

**Benchmark (Pop!_OS, 4-core, 100K nodes / 1M edges):**

| Operation | Time |
|---|---|
| CSR graph build + cache write | 178 ms |
| Cache reload (threshold-only change) | 68 ms |
| Deterministic cascade — SciPy sparse | 28 ms |
| Deterministic cascade — Numba JIT | 51 ms (cached compilation) |
| 100 stochastic MC trials, 4 workers | 40 s |
...
├── graph.py                                # Graph generation (NetworkX → NumPy) — legacy
├── propagation.py                          # Deterministic propagation engine (Tier 1) — legacy
├── stochastic_propagation.py               # Logistic stochastic extension (Tier 2) — legacy
├── metrics.py                              # Cascade size, fragility, RMSE, MAPE, Spearman
├── monte_carlo.py                          # Monte Carlo harness + CI utilities (Tier 2) — legacy
...
├── requirements.txt
├── ingestion.py                            # ★ NEW: CSV → integer IDs (pd.factorize)
├── graph_sparse.py                         # ★ NEW: CSR sparse graph + pickle lazy cache
├── propagation_fast.py                     # ★ NEW: Numba JIT @njit(parallel=True) + sparse matvec
├── monte_carlo_parallel.py                 # ★ NEW: ProcessPoolExecutor 4-core Monte Carlo
├── progress.py                             # ★ NEW: ProgressTracker + callback protocol
...
Install the runtime/test dependencies (now includes `numba` and `joblib` for the fast path):

```bash
pip install -r cascade_engine/requirements.txt
```

Or to use the exact pinned versions (recommended for reproducibility):
